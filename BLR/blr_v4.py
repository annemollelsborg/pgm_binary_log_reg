...
degF = 5.  # degrees of freedom, e.g. 1 for Cauchy, >1 for general t-distribution
tau = 1
...
def model(X, n_cat, y=None):
    ...
    mu_beta = pyro.sample("mu_beta", dist.StudentT(
    df=torch.full((n_features, 1), degF),
    loc=torch.zeros(n_features, 1),
    scale=torch.ones(n_features, 1)).to_event(2))

    sigma_beta = pyro.sample(
    "sigma_beta",
    dist.HalfCauchy(scale=tau * torch.ones(n_features, 1)).to_event(2)
)

    beta = pyro.sample("beta", dist.Normal(loc=mu_beta, scale=sigma_beta).to_event(2)) # Priors, accuracy = .575
